...
//Funzione che fa allineamento
//La funzione restituisce un puntatore alla struttura dati CommonKmerHead nella riga in cui è allocata la memoria per commonKmerHead
	...
	for(i = startReadIndex; i < readSetHead->readCount; i++){
		...
		for(long int j = 0; j < readLength - kmerLength + 1;j = j + step){
			...
			if(hashIndex!=-1){ //Se l'indice hash è valido (hashIndex!=-1)
				...
				//Se viene trovato, inserisce il k-mer nella struttura dati commonKmerHead.
				//printf("\nkmer di riferimento: %s\n",kmer1);

				//printf("\n funzione  InsertCommonToTwoReadAligningHead\n");
				InsertCommonToTwoReadAligningHead(commonKmerHead, kmerReadNodeHead, kmerHashTableHead, hashIndex, readIndex, j, 0); //lo zero indica il revers
			}	
		}
