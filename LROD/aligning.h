...
typedef struct CommonKmer{
	long int readIndex; //indice della read in cui si trova il common kmer
	//left Position è il punto di partenza del kmer nella rella read (j)
	long int leftPosition; // posizione del kmer nella kmerReadNode[i]
	long int rightPosition; //posizione del kmer nella read[i] considerata
	bool orientation; // se 1 -> kmer è in forward, se 0 è in reverse.
}CommonKmer;
...
typedef struct CommonKmerHead{
	...
	long int size; //aggiunto da me per capire la size totale, poiche ad ogni read
}CommonKmerHead;

typedef struct CommonKmerHeadArray{ //non usata
...
typedef struct AdjGraph{
	long int dataLeft; //assumono il valore di leftPosition di commonKmer, quindi KmerReadNode[i]
	long int dataRight;//position del kmer nella read[i]
	bool visit;
}AdjGraph;
 
typedef struct AdjGraphHead{
	AdjGraph * graph; //lista di grafi
